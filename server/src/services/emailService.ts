...
export class EmailService {
  ...
  async sendEmail(options: EmailOptions): Promise<boolean> {
    ...
    try {
      const msg = {
        to: options.to,
        from: env.email.from || 'no-reply@unityvault.com',
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <div class="warning">
                <strong>Important:</strong> Your activation link and OTP expire in <strong>24 hours</strong> (at <strong>${expiryDate}</strong>).
              </div>
              ...
              <p style="color: #b45309; font-weight: 600;">After 24 hours, both the activation link and OTP will expire and you'll need a new invitation.</p>
  ...
  /**
   * Verify SendGrid configuration is set
   */
  async verify(): Promise<boolean> {
    if (!env.email.sendgridApiKey || !env.email.from) {
      console.error("SendGrid API key or FROM_EMAIL is not configured.");
      return false;
    }
    // Optionally, try to send a test email here for deeper verification
    return true;
  }
}
